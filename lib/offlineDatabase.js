...
class OfflineDatabaseService {
  ...
  /**
   * Find anime mappings by multiple MAL IDs
   * 
   * @param {Array<number>} malIds - Array of MyAnimeList IDs
   * @returns {Promise<Array>} Array of anime mappings
   */
  async findMappingsByMalIds(malIds) {
    if (!malIds || malIds.length === 0) {
      return [];
    }
    return await AnimeMapping.find({ malId: { $in: malIds } });
  }
