...
class AnimeSyncService {
  constructor(userId, malAccessToken) {
        ...
        try {
          ...
          const response = await this.malApi.getAnimeList({
            animeList: [
              ...
              'nsfw', 'genres', 'media_type', 'status', 'num_episodes', 'start_season',
  ...
  async processAnimeEntry(animeEntry, options = {}) {
    ...
    try {
      ...
      // Prepare anime data for database
      const animeDbData = {
        malId: animeData.id,
        title: animeData.title,
        alternative_titles: animeData.alternative_titles || {},
        main_picture: animeData.main_picture || {},
        start_date: animeData.start_date ? new Date(animeData.start_date) : null,
        end_date: animeData.end_date ? new Date(animeData.end_date) : null,
        ...
        start_season: animeData.start_season || {},
      ...
      // Log error to anime record if it exists
      try {
        const anime = await Anime.findOne({ malId: animeEntry.node?.id })
        if (anime) {
          anime.syncMetadata.syncErrors.push({
            error: error.message,
            timestamp: new Date(),
            resolved: false,
          })
