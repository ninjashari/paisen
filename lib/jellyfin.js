...
class JellyfinApi {
  ...
  async getAnimeItems(userId) {
    try {
      ...
      // For each anime series, get detailed episode information
      const detailedAnimeItems = await Promise.all(
        animeItems.map(async (series) => {
          try {
            // Get episodes for this series to calculate watched count
            const episodesResponse = await this.http.get(`/Users/${userId}/Items`, {
              params: {
                ParentId: series.Id,
            ...
            const watchedEpisodes = episodes.filter(ep => ep.UserData?.Played).length
            ...
            console.warn(`Failed to get episode details for ${series.Name}:`, error.message)
            return series
          }
        })
      )
  ...
  extractAnimeInfo(jellyfinItem) {
    ...
    return {
      ...
      anidbId: providerIds.AniDb ? parseInt(providerIds.AniDb) : null,
