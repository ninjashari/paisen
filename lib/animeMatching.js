...
class AnimeMatchingService {
  ...
  async findMalMatch(jellyfinAnime) {
    ...
    try {
      // Strategy 1: AniDB ID match (highest priority)
      ...
      // Strategy 2: Direct MAL ID match
  ...
  /**
   * Match anime by AniDB ID by looking up in our local mapping database.
   *
  ...
  async matchByAnidbId(anidbId) {
    try {
      const localAnime = await this.findAnimeByExternalId('anidb', anidbId);
      if (localAnime && localAnime.malId) {
        // We have a match in our database, now get the full details from MAL API
        // This ensures we have the latest num_episodes, etc.
        const malAnime = await this.malApi.getAnimeDetails(localAnime.malId);
        if (malAnime && malAnime.data) {
          return {
            ...malAnime.data,
